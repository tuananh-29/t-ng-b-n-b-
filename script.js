...
// --- TẠO TIMELINE GSAP (NHƯNG TẠM DỪNG - PAUSED: TRUE) ---
var tl = gsap.timeline({ paused: true })
    ...
    // --- HIỆN LỜI CHÚC Ở CUỐI ---
    .to('.message', { autoAlpha: 1, duration: 2, ease: "power2.out" }, "+=0.5");

// --- SỰ KIỆN CLICK HỘP QUÀ ---
document.getElementById('gift-container').addEventListener('click', function() {
    // 1. Ẩn Hộp quà và Lời dẫn
    document.getElementById('gift-layer').style.transition = "opacity 1s ease";
    document.getElementById('gift-layer').style.opacity = 0;
    
    setTimeout(() => {
        document.getElementById('gift-layer').style.display = "none";
        
        // 2. Hiện container hoa
        document.getElementById('flower-container').style.opacity = 1;
        
        // 3. Bắt đầu chạy hiệu ứng hoa nở
        tl.play();
    }, 1000); // Đợi 1 giây để hộp quà mờ dần rồi mới chạy hoa
});